var PROVIDER = (function() {
    var internal = {
        ...
        onReceiveData: function(queryTerms, processedData, items) {
            LOGGING.logNewSearch(queryTerms);
        ...
        EEXCESS: {
            url: null,
            request: function(terms, action) {

                chrome.runtime.sendMessage(chrome.i18n.getMessage('@@extension_id'), {method: {parent: 'model', func: 'query'}, data: [{weight:1,text:terms}]});

                var updateFacetScape = function(response) {
                    if((typeof response == "undefined") || response == null) {
                        d3.select("div#facetScape").text("no data available");
                    } else {
        ...
        buildFacetScape: function(queryTerms, provider, domElem, width, height) {
            ...
            switch(provider) {
                ...
                case PROVIDER.names.EEXCESS:
                    internal.EEXCESS.request(queryTerms);
