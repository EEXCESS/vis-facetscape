function facetScape(domElem, iwidth, iheight, ifacets, queryResultItems, term) {
    ...
    var INTERACTION = (function() {
        ...
        return {
            ...
            onMouseClickFunction: function(d, i) {
                ...
                if(d.isSelected) {
                    LOGGING.logInteraction(facetName, "selection");
                    ...
                    LOGGING.logInteraction(facetName, "deselection");
                    var id = tagSelection[facetName].indexOf(d.word);
                    tagSelection[facetName].splice(id, 1);
                    if(tagSelection[facetName].length == 0) {
                        delete tagSelection[facetName];
            ...
            onMouseDragStartFacetName: function(d, i) {
                LOGGING.logInteraction(d.name, "move");
                d.weight = 1;
            ...
            onMouseDragEndFacetName: function(d, i) {
                ...
                if(facet.centroid.X > width) {
                    if(spareFacets.indexOf(this.parentNode) == -1 && facetData.length > 1) {
                        ...
                        LOGGING.logInteraction(facet.name, "exclude");
                ...
                spareFacets.forEach(function(d,i) {
                    ...
                    if(typeof facetToDrop !== "undefined") {
                        if(facetToDrop.name === name) {
                            LOGGING.logInteraction(facetToDrop.name, "move");
                            LOGGING.logInteraction(facetToDrop.name, "include");
    ...
    function FSResultLayout() {
        root.append("div").attr("id", "RS_ResultList").style("width", svgWidth-2+"px").attr("class", "resultList");
        RENDERING.drawResultList();
    }
        ...
        return {
            drawResultList: function() {
                ...
                singleResultNode.append("a").attr("class", "resultList-item-title")
                    .attr("href", function(d, i) { return PROVIDER.getEconbizRecordUrl() + d.id;})
                    .attr("target", "_blank")
                    .text(function(d,i) {
                ...
                var secDesc = singleResultNode.append("div").attr("class", "resultList-item-description")
                    .text(function(d, i) {
                        var descStr = "";
                        if(d['type']) {
                            descStr += (d['type'] instanceof Array) ? d['type'].join(", ") : d['type'];
                            descStr += " - ";
                        }
                        if(d['publisher']) {
                            descStr += (d['publisher'] instanceof Array) ? d['publisher'].join(", ") : d['publisher'];
                            descStr += " - ";
                        }
                        if(d['person']) {
                            descStr += (d['person'] instanceof Array) ? d['person'].join(", ") : d['person'];
                        }
                        return descStr;
                    });
            },
...
//                    .attr("alignment-baseline", "central")
            ...
            refreshFacetScape: function() {
                ...
                // TODO: after user interaction some polygons may no longer be well defined. Fix it!
                if(polys.length > 0) {
